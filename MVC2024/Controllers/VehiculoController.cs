...
namespace MVC2024.Controllers
{
    public class VehiculoController : Controller
    {
        ...
        {
            ViewBag.Sucursal=Contexto.Sucursales.ToList();
            ViewBag.marcas = Contexto.Marcas.ToList();
            ViewBag.Extras = Contexto.Extras.ToList();

            var lista = Contexto.Vehiculos.Include(v => v.Serie).Include(v => v.VehiculoExtras).ToList();
        ...
        public ActionResult Create()
        {
            ...
            ViewBag.Extras = new MultiSelectList(Contexto.Extras, "ID", "NomExtra");
            return View();
        }
        ...
        public ActionResult Create(VehiculoModelo vehiculo)
        {
            ...
            foreach (var xtrasID in vehiculo.ExtrasSeleccionados) 
            {
                ...
                Contexto.VehiculosExtra.Add(obj);          
            }
            Contexto.SaveChanges();
            try
            {
        ...
        public ActionResult Edit(int id)
        {
            ...
            vehiculo.ExtrasSeleccionados = Contexto.VehiculosExtra.Where(ve => ve.vehiculoID == id).Select(ve =>ve.extraID).ToList();
            ViewBag.xtraList = new MultiSelectList(Contexto.Extras, "ID", "NomExtra","ExtrasSeleccionados");
            return View(vehiculo);
        }
        ...
        public ActionResult Edit(int id, VehiculoModelo cocheDatosNew)
        {
            ...
            var extrasActuales= Contexto.VehiculosExtra.Where(ve => ve.vehiculoID == id);
            //Un result set es el conjunto de un consulta sql
            foreach (var extraAEliminar in extrasActuales)
                Contexto.VehiculosExtra.Remove(extraAEliminar);

            //Creas un Objeto nuevo con los valores atraves de la tabla intermedia
            foreach (var extraAAñadir in cocheDatosNew.ExtrasSeleccionados) 
               {
                var ObjVehiculoExtra = new VehiculoExtraModelo()//creas un Objeto con metodo constructor
                {
                    extraID = extraAAñadir,
                    vehiculoID = cocheDatosNew.Id
                };

                Contexto.VehiculosExtra.Add(ObjVehiculoExtra);
               }
            Contexto.SaveChanges();
            try
            {
